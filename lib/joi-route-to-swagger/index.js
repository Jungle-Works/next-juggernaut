...
const DOC_ROOT_TEMPLATE = {
  swagger: '2.0',
  info: {
    description: 'CL BASE PROJECT MICROSERVICES',
    version: '1.0.0',
    title: 'CL BASE PROJECT'
  },
  host: 'localhost',
  basePath: '/',
  tags: [],
...
function addRequestHeadersParams(route, validators) {
  if (validators && validators.headers) {
    const queryParams = joi2json(validators.headers);
    _.forEach(queryParams.properties, (value, field) => {
      let description = value.description ? value.description : '';
      const example = value.examples && value.examples.length > 0 ? value.examples[0] : undefined;
      if (example) {
...
function buildSwaggerRequest(docEntity, moduleId, basePath, routeDef) {
  ...
  const actionName = action.name+routeDef.path+routeDef.method;
  if (!actionName) {
    return;
  }
  ...
  swaggerReq.tags.push(routeDef.tags);
  ...
  addRequestHeadersParams(swaggerReq, validators);
...
function convert(allModuleRoutes, docSkeleton) {
  ...
  docEntity.host = allModuleRoutes[0].host || DOC_ROOT_TEMPLATE.host;
  ...
  fs.writeFileSync('./node_modules/next-juggernaut/lib/self.json', JSON.stringify(docEntity, null, 2));
  return docEntity;
}
